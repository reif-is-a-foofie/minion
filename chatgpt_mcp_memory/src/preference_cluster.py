"""Offline clustering over chunk embeddings → preference_clusters rows + telemetry."""
...
def _cosine_kmeans(X: np.ndarray, k: int, *, seed: int = 42) -> np.ndarray:
    """X: (n, d) L2-normalized. Returns integer labels (n,)."""
    n, d = X.shape
    k = int(max(2, min(k, n)))
...
def _heuristic_label(samples: List[str]) -> Tuple[str, str]:
    """Short label + summary from cluster member texts (no LLM)."""
